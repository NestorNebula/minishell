...
static void		test_parse_tokens_valid(t_parsing_table parsing_table,
					t_token_type *token_types, size_t size,
					const char *test);

static void		test_parse_tokens_invalid(t_parsing_table parsing_table,
					t_token_type *token_types, size_t size,
...
static void		test_parse_tokens(void)
{
	...
	if (parsing_table == NULL)
		return ;
	test_parse_tokens_valid(parsing_table, (t_token_type[]){T_WORD, T_WORD}, 2, "parse_tokens (word tokens)");
	test_parse_tokens_valid(parsing_table, (t_token_type[]){T_REDIR_IN, T_WORD}, 2, "parse_tokens (redir input token)");
	test_parse_tokens_valid(parsing_table, (t_token_type[]){T_REDIR_OUT, T_WORD}, 2, "parse_tokens (redir output token)");
	...
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_HEREDOC}, 1, T_HEREDOC, "parse_tokens (heredoc token alone)");
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_PIPE}, 1, T_PIPE, "parse_tokens (pipe token alone)");
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_PIPE, T_WORD}, 2, T_PIPE, "parse_tokens (pipe as first token)");
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_WORD, T_PIPE}, 2, T_PIPE, "parse_tokens (pipe as last token)");
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_HEREDOC, T_PIPE, T_WORD}, 3, T_PIPE, "parse_tokens (pipe token before complete command)");
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_WORD, T_PIPE, T_PIPE, T_WORD}, 4, T_PIPE, "parse_tokens (double pipe token)");
	test_parse_tokens_invalid(parsing_table, (t_token_type[]){T_REDIR_IN, T_REDIR_IN, T_WORD}, 3, T_REDIR_IN, "parse_tokens (double redir input token)");
